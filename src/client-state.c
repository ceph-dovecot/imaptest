...
static void auth_plain_callback(struct imap_client *client, struct command *cmd, const struct imap_arg *args,
...
int imap_client_append_continue(struct imap_client *client) {
  ...
  if ((client->capabilities & CAP_MULTIAPPEND) != 0 && states[STATE_APPEND].probability_again != 0 &&
...
int imap_client_append(struct imap_client *client, const char *args, bool add_datetime, command_callback_t *callback,
  ...
  *cmd_r = NULL;
  ...
  i_assert(client->append_stream == NULL);
  client->append_stream = mailbox_source_get_next(client->storage->source, &vsize, &t, &tz);
