...
static void client_wait_connect(struct client *client) {
  ...
  if (conf.port == 993) {
    ...
    if (io_stream_create_ssl_client(ssl_ctx, conf.host, &ssl_set, &client->input, &client->output,
                                    &client->ssl_iostream, &error) < 0)
      i_fatal("Couldn't create SSL iostream: %s", error);
    (void)ssl_iostream_handshake(client->ssl_iostream);
  }
  if (conf.rawlog) {
    if (iostream_rawlog_create_path(t_strdup_printf("rawlog.%u", client->global_id), &client->input, &client->output) !=
        0)
